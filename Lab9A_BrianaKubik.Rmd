...
Regular patterns are strings of characters that define a search pattern. Typically patterns are used as string-searching algorithms for "find" or "find and replace" operations on strings. 

# Base R Functions

The base R functions for dealing with regular expressions are

* `grep()`, `grep1()` - these functions search fr matches of a regular expression/pattern in a character vector. `grep()` returns the indices into the character vector that contain a match or the specific strings that happen to have the match. `grep1()` returns a `TRUE/FALSE` vactor indicating which elements of the character vector contain a match.
...
* `regexec()` - Searches a character vector for a regular expression, much like `regexpr()`, but will additionally return the locations of any parenthesized sub-expressions.

# The **`stringr`** package

The **`stringr`** package is a part of the **`tidyverse`** collection and wraps the underlying **`stringi`** package in a series of convenient functions. There is a straightforward mapping from the base R functions to the **`stringr`** functions. Generally, the first argument is the data, the second argument is the regular expression, and the optional arguments are after.

`str_subset()` is like `grep(value = TRUE)` and returns a character vector of strings that contain a given match.

`str_detect()` is essentially `grep1()`.

`str_extract()` does the same thing as `regexpr()` and `regmatches()` by extracting the matches from the output.

`str_match()` is the same as `regexec()` which provides a matrix containing the parenthesized sub-expressions.

# 14.3 Matching patterns wih regular expressions

To learn about regular expressions, we will use `str_view()` and `str_view_all()`. These functions take a character vector and a regular expression and show you how they match.
...
The simplest patterns match the FIRST exact string in a word:
...
If `.` matches any character, how would you match the character "."? We will need to use an escape to tell the regular expression that we want to match it exactly. Like strings, regexps use the backslash, \ , to escape special behavior. So `\.` can be used to match a ".". However, `\` is an escape symbol in strings so to create the regular expression `\.`, we must use the string "\\."
...
If we are using `\` as an escape character in regular expressions, how do we match a literal `\`? We would need `\\\\` to match 1.
...
<br/><br/> **A single `\` will be used to escape the next character in the string. `\\` uses the first `\` to escape the next `\` in the string, resulting in a `\` in the regular expression which will escape the next character in the regexp. Lastly, `\\\` will produce 1 `\` form the first 2 `\\` but the third will be used to escape the following character.**
...
<br/><br/> **This regular expression will match patterns of 3 dots with any character after each one.**
...
Regular expression will match any part of a string. It is useful to anchor the regular expression so that it matches from the start or end of the string. We can use:
...
There are a number of special patterns that match more than one character like `.` which matches any character. HEre are some other useful ones:

* `\d` - matches any digit.
* `\s` - matches any whitespace (i.e. space, tab, newline).
...
1. Create regular expressions to find all words in `stringr::words` that:
...
str_subset(c("ed", stringr::words), "(^|[^e])ed$") #if we want to include "ed"
```
...
<br/><br/> **`?` is the same as {0,1}, `+` is the same as {1,}, and `*` is the same as {0,}. To demonstrate this, here are the examples from the chapter:** 
...
We learned that parentheses can be used to disambiguate complex expressions. They can also create a numbered capturing group. A capturing groups stores the part of the string matched by the part of the regular expression inside the parentheses. You can refer to the same text as previously matched by a capturing group with backreferences, like `\1`, `\2` etc. For example, the following regular expression finds all fruits that have a repeated pair of letters.
...
* `(.)\1\1` - **The same character repeated 3 times (i.e. "aaa")**
* `"(.)(.)\\2\\1"` - **A pair of characters followed by the same pair of characters but in reverse (i.e. "abba")**
* `(..)\1` - **Any 2 characters followed by this same characters again (i.e. "abab")**
* `"(.).\\1.\\1"` - **A character followed by any character, the original character, any character, the original character again (i.e. "abaca")**
* `"(.)(.)(.).*\\3\\2\\1"` - **Any three characters followed by 0 or more of any character, then the original 3 characters in backward order (i.e. "abc1cba" or "abc1akljgmwojajelwecba" or "abccba")**
