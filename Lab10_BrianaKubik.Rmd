...
**In R, writing a question mark (?) followed by a function will pull up the help page on that function.**
...
**The `element` argument in the `queryMETA()` function will summarize all the information about that element. "AD" stands for a 'FORMAT' acronym that describes the allelic depths for the ref and alt alleles in the order listed.** 
...
**In R, the `head()` function will display the first 5 rows of data and the `tail()` function will display the last 5 rows of data.**
...
**This does not seem to be useful as it **
...
## *chromR* object - using *chromR* to locate unusual features in a genome. 
...
**The sampling size is too small. The population counts only range from 1-6.**
...
**You can easily do this by changing the x-axis range using `xlim = `.**
